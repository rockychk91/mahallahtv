...
if (typeof Promise === 'undefined') {
    window.Promise = function (executor) {
        ...
        self.catch = function () { return self; };
        executor(function (val) {
            if (self._then) self._then(val);
        }, function () {});
    };
}
...
OfflineLicenseSystem.prototype.startDeviceHeartbeat = function() {
    ...
    setTimeout(function() {
        try {
            ...
            localStorage.setItem('last_heartbeat_time', new Date().getTime());
...
OfflineLicenseSystem.prototype.resumeHeartbeatIfNeeded = function() {
    ...
    if (lastHeartbeatTime && savedLicenseKey && savedDeviceId) {
        ...
        var timeDiff = new Date().getTime() - parseInt(lastHeartbeatTime);
...
OfflineLicenseSystem.prototype.getCachedValidation = function(key) {
    ...
    if (cache && (new Date().getTime() - cache.timestamp < this.cacheTimeout)) {
        return cache.data;
    }
    return null;
};
